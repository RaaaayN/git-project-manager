    ...
    if managed_files_raw is None:
        ...
        for template_path in sorted(template_root.glob("*.md")):
            managed_files.append(
                ManagedFile(template=template_path.name, target=template_path.name)
            )
    else:
...
def _gemini_generate_json(prompt: str, llm_config: LLMConfig) -> dict[str, Any] | None:
    ...
    if not isinstance(first_part, dict):
        return None
    text = _safe_text(first_part.get("text"))
...
def _format_next_steps_markdown(next_steps: list[dict[str, str]]) -> list[str]:
    if not next_steps:
        return ["- [ ] Step: Aucun next step prioritaire detecte", "      Owner: TBD", "      Depends on: none", "      Priority: P2", "      Status: Pending", "      Evidence: none"]
...
def _build_adr_entry(adr_number: int, adr_draft: dict[str, Any]) -> str:
    decision = _safe_text(adr_draft.get("decision"))
    rationale = adr_draft.get("rationale")
    impact = adr_draft.get("impact")
    reference = _safe_text(adr_draft.get("reference"))
    date_value = _safe_text(adr_draft.get("date"))
...
def _gemini_refine_next_steps(
    ...
    for item in candidate_steps:
        ...
        owner = _safe_text(item.get("owner")).strip()
        depends_on = _safe_text(item.get("depends_on")).strip()
        priority = _safe_text(item.get("priority")).strip()
        status = _safe_text(item.get("status")).strip()
        evidence = _safe_text(item.get("evidence")).strip()
...
def _infer_event_name_from_payload(payload: dict[str, Any]) -> str:
    object_kind = _safe_text(payload.get("object_kind")).lower()
...
def normalize_gitlab_event(event_name: str, payload: dict[str, Any]) -> NormalizedEvent:
    ...
    author = _safe_text(user_data.get("username") or user_data.get("name") or payload.get("user_name"))
    ...
    if event_type == "merge_request":
        action = _safe_text(object_attributes.get("action") or payload.get("action") or "unknown")
        state = _safe_text(
            object_attributes.get("state") or ("merged" if action in {"merge", "merged"} else "unknown")
        )
        title = _safe_text(object_attributes.get("title") or payload.get("title"))
        url = _safe_text(object_attributes.get("url") or object_attributes.get("target"))
        iid = object_attributes.get("iid")
        reference = f"!{iid}" if isinstance(iid, int) else "merge_request"
    elif event_type == "issue":
        action = _safe_text(object_attributes.get("action") or payload.get("action") or "unknown")
        state = _safe_text(object_attributes.get("state") or "opened")
        title = _safe_text(object_attributes.get("title") or payload.get("title"))
        url = _safe_text(object_attributes.get("url"))
        iid = object_attributes.get("iid")
        reference = f"#{iid}" if isinstance(iid, int) else "issue"
    elif event_type == "pipeline":
        status = _safe_text(object_attributes.get("status") or payload.get("status") or "unknown")
        action = status
        state = status
        ...
        action = _safe_text(payload.get("action") or "unknown")
        state = _safe_text(payload.get("state") or "unknown")
        title = _safe_text(payload.get("title") or payload.get("object_kind") or "Unsupported event")
...
def extract_context(repo_root: Path, config: AgentConfig, event: NormalizedEvent) -> dict[str, Any]:
    ...
    if event.event_type == "issue":
        ...
        if isinstance(object_attributes, dict):
            issue_description = _safe_text(object_attributes.get("description"))
...
def detect_gaps(event: NormalizedEvent, context: dict[str, Any]) -> list[Gap]:
    ...
    if event.event_type == "issue":
        description = _safe_text(context.get("issue_description", ""))
...
def _run_bootstrap(args: argparse.Namespace) -> int:
    ...
    results = create_missing_files(config_path.parent.resolve(), config, dry_run=dry_run)
    _print_results(results, dry_run=dry_run, show_diff=not args.no_diff)
    return 0


def _run_process_event(args: argparse.Namespace) -> int:
    config_path = Path(args.config).resolve()
    config = load_config(config_path)
    payload = _read_payload(args)
    event_name = args.event_name or _infer_event_name_from_payload(payload)
    plan = process_event_pipeline(config_path.parent.resolve(), config, event_name, payload)
    print(json.dumps(_event_plan_to_dict(plan), indent=2, ensure_ascii=True))
    return 0


def _sync_docs_from_plan(
    repo_root: Path,
    config: AgentConfig,
    plan: EventPlan,
    dry_run: bool,
    ...
    adr_draft = plan.adr_draft
    if adr_draft:
        reference = _safe_text(adr_draft.get("reference")).strip()
...
def _run_sync_docs(args: argparse.Namespace) -> int:
    config_path = Path(args.config).resolve()
    config = load_config(config_path)
    payload = _read_payload(args)
    event_name = args.event_name or _infer_event_name_from_payload(payload)
...
def _make_webhook_handler(
    ...
    class GitLabWebhookHandler(BaseHTTPRequestHandler):
        def do_POST(self) -> None:  # noqa: N802
            ...
            event_name = _safe_text(self.headers.get("X-Gitlab-Event"))
...
def _run_serve_webhook(args: argparse.Namespace) -> int:
    config_path = Path(args.config).resolve()
    config = load_config(config_path)
    repo_root = config_path.parent.resolve()
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    known_commands = {"bootstrap", "process-event", "sync-docs", "serve-webhook"}
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        if command == "process-event":
            return _run_process_event(args)
        if command == "sync-docs":
            return _run_sync_docs(args)
        if command == "serve-webhook":
            return _run_serve_webhook(args)
